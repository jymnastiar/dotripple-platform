...
export function TypingAnimation({
  ...
  const renderText = () => {
    ...
    const currentWord = wordsToAnimate[currentWordIndex] || ""
    const matchIndex = currentWord.indexOf(highlightText)

    if (matchIndex !== -1) {
      const beforeHighlight = displayedText.slice(0, matchIndex)
      const highlightPart = displayedText.slice(
        matchIndex,
        matchIndex + highlightText.length
      )
      const afterHighlight = displayedText.slice(
        matchIndex + highlightText.length
      )
      ...
      return (
        <>
          {beforeHighlight}
          {highlightPart && (
            <span className={highlightClassName}>{highlightPart}</span>
          )}
          {afterHighlight}
        </>
      )
